...
class ASR(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        #    log_probabilities=predictions["sentiment_logprobs"],
        #    targets=sentiments.squeeze(-1),
        ...
        if stage != sb.Stage.TRAIN:
            ...
            #self.uar = balanced_accuracy_score(sentiments.cpu(), topk.cpu())
    ...
    def evaluate_data(
        ...
        with torch.no_grad():
            ...
            for batch in dataset:
                ...
                #    np.save(os.path.join("../data/custom_features_augmented_autoencoder/dev", filenames[i] + ".npy"), prediction_embedding[i])
...
def data_prep(data_folder, hparams):
    ...
    train_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "train_vggish_augmented.json"), replacements={"data_root": data_folder})
    valid_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "devel_vggish_augmented.json"), replacements={"data_root": data_folder})
    test_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "test_vggish.json"), replacements={"data_root": data_folder})
...
def main(device="cuda"):
    ...
    #sent2idx = {"Prolongation": 0, "no_disfluencies": 1, "Fillers": 2, "Block": 3, "Modified": 4, "SoundRepetition": 5, "WordRepetition": 6, "Garbage": 7}
    #train_targets = []
    #weights = torch.tensor([0.0054, 0.0012, 0.0048, 0.0032, 0.0014, 0.0059, 0.0188, 0.0192])

    #with open("../data/train_vggish.json", "r") as f:
    #    data = f.readlines()
    #data = json.loads(data[0])
    #for line in data:
    #    sentiment = data[line]["sentiment"]
    #    train_targets.append(sent2idx[sentiment])

    #sample_weights = weights[train_targets]
    #sample_weights = sample_weights.cpu().detach().numpy()
    #
    #sampler = sb.dataio.sampler.ReproducibleWeightedRandomSampler(weights=sample_weights, replacement=True, num_samples=len(sample_weights))
    ...
    #        train_data,
    #        valid_data,
    #        train_loader_kwargs={"batch_size": hparams["dataloader_options"]["batch_size"], "shuffle": False, "sampler": sampler},
