...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        predictions = {"sentiment_logprobs": self.hparams.log_softmax(encoder_out), "sentiment_logits": encoder_out}
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            #prediction_logprobs = torch.log_softmax(predictions["sentiment_logprobs"], dim=1)
            #print(torch.argmax(prediction_logprobs, dim=-1))
    ...
    def evaluate_data(
        ...
        with torch.no_grad():
            label2idx = {"fear": 0, "pleasure": 1, "anger": 2, "achievement": 3, "pain": 4, "surprise": 5}
        ...
        #with open("output/true_labels.csv", "w") as f:
        #    for i in range(len(pred_sentiments)):
        #        f.write(filenames_array[i] + "," + str(idx2label[true_sentiments[i]]))
        #        f.write("\n")
        ...
        #np.save("output/probs_devel_m_f_tcn.npy", np.exp(pred_prob))
...
def data_prep(data_folder, hparams):
    ...
    train_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "train_vggish_m_f.json"), replacements={"data_root": data_folder})
    valid_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "dev_vggish_m_f.json"), replacements={"data_root": data_folder})
    test_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "dev_vggish_m_f.json"), replacements={"data_root": data_folder})
...
def main(device="cuda"):
    ...
    # Dataset creation
    #change this to your VGGish features directory
    train_data, valid_data, test_data = data_prep("../../../../Vocalization/data", hparams)
