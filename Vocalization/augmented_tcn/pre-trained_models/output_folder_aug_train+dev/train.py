...
class ASR(sb.Brain):
    ...
    def evaluate_data(
        ...
        #        f.write(filenames_array[i] + "," + str(idx2label[pred_sentiments[i]]))
        #        f.write("\n")
        ...
        np.save("output/predicted_whole_data_tcn.npy", pred_sentiments)
        np.save("output/true_whole_data_tcn.npy", true_sentiments)
        #np.save("output/probs_test_whole_data_tcn.npy", np.exp(pred_prob))
...
def data_prep(data_folder, hparams):
    ...
    train_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "train_dev_vggish_augmented.json"), replacements={"data_root": data_folder})
    valid_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "dev_vggish_augmented.json"), replacements={"data_root": data_folder})
    test_data = sb.dataio.dataset.DynamicItemDataset.from_json(json_path=os.path.join(data_folder, "dev_vggish_augmented.json"), replacements={"data_root": data_folder})
